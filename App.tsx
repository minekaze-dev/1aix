...
export default function App() {
  ...
  const trendingNewsForMarquee = useMemo(() => {
    if (articles.length === 0) {
        return ["SISTEM DATABASE 1AIX TELAH AKTIF SEPENUHEYA", "SELURUH HARGA SRP BERSUMBER DARI STORE RESMI"];
    }
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[1001] flex flex-col items-center">
        ...
        >
          <svg className="absolute inset-0 w-full h-full text-zinc-900 drop-shadow-2xl" viewBox="0 0 80 32" preserveAspectRatio="none">
            <path d="M 0,32 L 80,32 L 64,4 Q 60,0 52,0 L 28,0 Q 20,0 16,4 L 0,32 Z" fill="currentColor" />
          </svg>
          <svg className={`w-5 h-5 text-white relative z-10 transition-transform duration-300 ${isBottomNavVisible ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3">
        ...
        <div className={`w-full bg-[#0b0b0b] text-white border-t border-zinc-800 transition-all duration-500 ease-in-out shadow-[0_-10px_25px_-5px_rgba(0,0,0,0.5)] ${isBottomNavVisible ? 'translate-y-0 opacity-100 h-16' : 'translate-y-full opacity-0 h-0 overflow-hidden'}`}>
          <div className="flex h-full items-center justify-around px-2">
            <button onClick={handleGoHome} className={`flex flex-col items-center gap-1 flex-1 transition-colors ${activeTab === 'Home' ? 'text-red-500' : 'text-zinc-500'}`}>
            ...
            <button onClick={handleGoToCompare} className={`flex flex-col items-center gap-1 flex-1 transition-colors ${activeTab === 'Bandingkan' ? 'text-blue-500' : 'text-zinc-500'}`}>
            ...
            <button onClick={handleGoToCatalog} className={`flex flex-col items-center gap-1 flex-1 transition-colors ${activeTab === 'Katalog' ? 'text-red-500' : 'text-zinc-500'}`}>
            ...
            {/* TKDN Always Present */}
            <button onClick={() => window.location.hash = '#/coming-soon'} className={`flex flex-col items-center gap-1 flex-1 transition-colors ${activeTab === 'Segera Rilis' ? 'text-blue-500' : 'text-zinc-500'}`}>
            ...
            {isAdmin && (
              <button onClick={() => { window.location.hash = '#/admin'; }} className={`flex flex-col items-center gap-1 flex-1 transition-colors ${(activeTab as string) === 'Admin' ? 'text-red-500' : 'text-zinc-500'}`}>
