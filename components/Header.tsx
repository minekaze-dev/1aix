        ...
        <header className="w-full max-w-[1000px] flex flex-col shadow-xl z-50 sticky top-0 lg:relative">
            ...
            <div className="bg-[#0b0b0b] text-white h-12 lg:h-16 flex items-center justify-between pl-4 lg:pl-6 pr-0 relative transition-all">
                ...
                <nav className="hidden lg:flex items-center h-full">
                    ...
                    {session ? (
                         <button onClick={onLogout} className="flex items-center justify-center w-10 h-full hover:bg-white/5 transition-all text-red-600 group" title="LOGOUT">
                        ...
                        <button onClick={onOpenLogin} className="flex items-center justify-center w-10 h-full hover:bg-white/5 transition-all text-red-600 group" title="MASUK">
                             <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><path d="M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                        </button>
                    )}
                </nav>
            ...
            {/* Mobile Search Overlay & Brand Drawer logic remains same ... */}
            {isSearchFocused && (
                ...
                <div className="fixed inset-0 z-[1000] lg:hidden animate-in fade-in duration-300">
                    ...
                    <div className="absolute right-0 top-0 bottom-0 w-[280px] bg-white shadow-2xl animate-in slide-in-from-right duration-500 flex flex-col">
                        <div className="bg-[#0b0b0b] p-6 flex justify-between items-center">
                            ...
                            <button onClick={() => setIsMobileMenuOpen(false)} className="text-red-500"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path d="M6 18L18 6M6 6l12 12"></path></svg></button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-4 space-y-1">
                            {brands.map(brand => (
                                <button key={brand} onClick={() => { onSelectBrand?.(brand); setIsMobileMenuOpen(false); window.location.hash = '#/katalog'; }} className={`w-full text-left p-4 rounded-sm text-sm font-black uppercase tracking-widest transition-colors ${selectedBrand === brand ? 'bg-red-600 text-white' : 'text-zinc-800 hover:bg-zinc-100'}`}>{brand}</button>
                            ))}
                        </div>
                        <div className="p-4 border-t border-zinc-100 flex flex-col gap-2">
                            <button onClick={() => { onSelectBrand?.(null); setIsMobileMenuOpen(false); window.location.hash = '#/katalog'; }} className="w-full p-4 bg-[#3b82f6] text-white text-[10px] font-black uppercase tracking-[0.3em] rounded-sm text-center">ALL BRAND</button>
                            <button onClick={() => { setIsMobileMenuOpen(false); window.location.hash = '#/home'; }} className="w-full p-4 bg-zinc-900 text-white text-[10px] font-black uppercase tracking-[0.3em] rounded-sm text-center">BACK TO HOME</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="w-full hidden lg:flex items-stretch h-12 border-b border-zinc-200">
                ...
                <div className="flex-1 bg-[#f1f5f9] flex justify-center items-center px-4 overflow-hidden self-stretch">
                    <div className="grid grid-cols-8 gap-x-6 gap-y-1 content-center">
                        {brands.map(brand => (
                            <button key={brand} onClick={() => onSelectBrand && onSelectBrand(brand)} className={`text-[10px] font-black uppercase tracking-tighter transition-colors text-center px-0.5 truncate flex items-center justify-center h-4 ${selectedBrand === brand ? 'text-red-600' : 'text-zinc-500 hover:text-zinc-800'}`}>{brand}</button>
                        ))}
                    </div>
                </div>
                <button onClick={() => window.location.hash = '#/katalog'} className="bg-[#3b82f6] px-5 text-white text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-blue-600 transition-colors flex-shrink-0 self-stretch">
