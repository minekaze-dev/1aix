...
const CatalogTab: React.FC<CatalogTabProps> = ({ 
    ...
    useEffect(() => { if (initialProduct) { setSelectedProduct(initialProduct); window.scrollTo({ top: 0, behavior: 'smooth' }); } else setSelectedProduct(null); }, [initialProduct]);
    ...
    const handleProductSelect = (phone: Smartphone) => { const slug = phone.model_name.toLowerCase().replace(/[^a-z0-9]+/g, '-'); window.location.hash = `#/katalog/${slug}`; };
            ...
            <div className="flex-grow">
                {selectedProduct ? (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <button onClick={() => { setSelectedProduct(null); window.location.hash = '#/katalog'; }} className="mb-8 flex items-center gap-2 text-[10px] font-black text-blue-600 uppercase tracking-widest hover:text-blue-800 transition-colors"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>KEMBALI KE {selectedBrand || 'KATALOG'}</button>
                        <div className="flex flex-col md:flex-row gap-8 items-end">
                            <div className="md:w-[220px] flex-shrink-0"><div className="bg-[#f8f9fa] border border-zinc-100 p-4 flex items-center justify-center rounded-sm"><img src={selectedProduct.image_url} alt={selectedProduct.model_name} className="w-full h-auto max-h-[240px] object-contain mix-blend-multiply drop-shadow-md" /></div><div className="mt-2 text-center"><span className="text-[7px] font-bold text-zinc-300 uppercase tracking-widest leading-none">Image: GSMArena</span></div></div>
                            <div className="flex-1 flex flex-col justify-end">
                                <div className="mb-6">
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-3"><div className="text-[10px] font-black text-red-600 uppercase tracking-[0.2em] leading-none">{selectedProduct.brand} Official</div><div className="bg-emerald-500 text-white text-[8px] font-black px-2 py-0.5 rounded-sm uppercase tracking-tighter leading-none">{selectedProduct.release_status}</div>{selectedProduct.market_category && <div className="bg-zinc-900 text-white text-[8px] font-black px-2 py-0.5 rounded-sm uppercase tracking-tighter leading-none border border-zinc-700">{selectedProduct.market_category}</div>}</div>
                                    ...
                                    <div className="py-8 mb-6 border-y border-zinc-100 flex flex-col items-center justify-center bg-zinc-50 rounded-sm"><div className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.3em] mb-2">ESTIMASI HARGA PASAR</div><div className="text-4xl font-black text-blue-600 tracking-tighter italic">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(selectedProduct.price_srp)}</div><div className="mt-4 text-[9px] font-bold text-zinc-400 uppercase tracking-widest text-center px-6 leading-relaxed max-w-[500px]">* HARGA PASAR DAPAT BERUBAH SEWAKTU-WAKTU TERGANTUNG WILAYAH DAN KEBIJAKAN TOKO RETAIL. DATA SRP RESMI SAAT PELUNCURAN.</div></div>
                                    <div className="flex flex-col gap-3"><div className="grid grid-cols-2 gap-3"><div className="flex items-center justify-center py-3 bg-zinc-900 text-white font-black uppercase text-[9px] tracking-widest rounded-sm shadow-md cursor-default">SPESIFIKASI</div><button onClick={() => onCompareProduct ? onCompareProduct(selectedProduct.id) : (window.location.hash = '#/bandingkan')} className="flex items-center justify-center py-3 bg-blue-600 text-white font-black uppercase text-[9px] tracking-widest hover:bg-blue-700 transition-colors rounded-sm shadow-md">COMPARE</button></div></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col items-center py-12 bg-white border-t border-zinc-100">
                            <h3 className="text-[12px] font-black text-zinc-900 uppercase tracking-[0.4em] mb-4">BERI PENILAIAN</h3>
                            ...
                            <div className="flex gap-16">
                                <div className="flex flex-col items-center"><button onClick={() => handleRating(selectedProduct.id, 'like')} className={`w-24 h-24 rounded-full bg-white shadow-xl flex items-center justify-center text-blue-500 hover:scale-110 active:scale-95 transition-all mb-6 group border ${userVote === 'like' ? 'border-blue-500 bg-blue-50 ring-4 ring-blue-100' : 'border-blue-50'}`}><svg className={`w-10 h-10 ${userVote === 'like' ? 'fill-blue-500' : 'fill-none'}`} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M14 9V5a3 3 0 00-3-3l-4 9v11h11.28a2 2 0 002-1.7l1.38-9a2 2 0 00-2-2.3zM7 22H4a2 2 0 01-2-2v-7a2 2 0 012-2h3"/></svg></button><span className={`text-2xl font-black tracking-tighter ${userVote === 'like' ? 'text-blue-600' : 'text-blue-600/40'}`}>{ratings[selectedProduct.id]?.likes || 0}</span></div>
                                <div className="flex flex-col items-center"><button onClick={() => handleRating(selectedProduct.id, 'dislike')} className={`w-24 h-24 rounded-full bg-white shadow-xl flex items-center justify-center text-zinc-400 hover:scale-110 active:scale-95 transition-all mb-6 group border ${userVote === 'dislike' ? 'border-zinc-800 bg-zinc-50 ring-4 ring-zinc-100' : 'border-zinc-50'}`}><svg className={`w-10 h-10 ${userVote === 'dislike' ? 'fill-zinc-800' : 'fill-none'}`} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M10 15v4a3 3 0 003 3l4-9V2H5.72a2 2 0 00-2 1.7l-1.38 9a2 2 0 002 2.3zm7-13h3a2 2 0 012 2v7a2 2 0 01-2 2h-3"/></svg></button><span className={`text-2xl font-black tracking-tighter ${userVote === 'dislike' ? 'text-zinc-800' : 'text-zinc-400'}`}>{ratings[selectedProduct.id]?.dislikes || 0}</span></div>
                            </div>
                        </div>
                    </div>
                ) : (
