...
const CatalogTab: React.FC<CatalogTabProps> = ({ 
    ...
    useEffect(() => {
        if (selectedProduct) {
            ...
            refreshRatingCounts(selectedProduct.id);
        }
            ...
            <aside className="w-[240px] flex-shrink-0 space-y-10 hidden lg:block">
                ...
                <div>
                    ...
                    <div className="space-y-0">
                        {items.slice(0, 3).map(phone => (
                            <div key={phone.id} onClick={() => handleProductSelect(phone)} className="flex items-center gap-6 group cursor-pointer border-b border-zinc-100/50 py-1.5 last:border-0 transition-all hover:bg-zinc-50/50">
                ...
                {selectedProduct ? (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="flex flex-col gap-3">
                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="flex items-center justify-center py-3 bg-zinc-900 text-white font-black uppercase text-[9px] tracking-widest rounded-sm shadow-md cursor-default">
                                                SPESIFIKASI
                                            </div>
                                            <button 
                        ...
                        <div className="flex flex-col items-center py-12 bg-white border-t border-zinc-100">
                            ...
                            {showLoginWarning && !session && (
                                <div className="w-full max-w-md bg-[#ef4444] text-white py-3 px-6 rounded-md mb-8 animate-in fade-in zoom-in-95 duration-300 shadow-lg text-center">
                                    <span className="text-[11px] font-black uppercase tracking-widest">MAAF, ANDA HARUS LOGIN TERLEBIH DAHULU.</span>
                                </div>
                            )}
                            ...
                            <div className="flex gap-16">
                                <div className="flex flex-col items-center">
                                    <button 
                                        ...
                                        className={`w-24 h-24 rounded-full bg-white shadow-xl flex items-center justify-center text-blue-500 hover:scale-110 active:scale-95 transition-all mb-6 group border ${userVote === 'like' ? 'border-blue-500 bg-blue-50 ring-4 ring-blue-100' : 'border-blue-50'}`}
                                    >
                                        <svg className={`w-10 h-10 ${userVote === 'like' ? 'fill-blue-500' : 'fill-none'}`} stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <span className={`text-2xl font-black tracking-tighter ${userVote === 'like' ? 'text-blue-600' : 'text-blue-600/40'}`}>
                                        {ratings[selectedProduct.id]?.likes || 0}
                                    </span>
                                </div>
                                <div className="flex flex-col items-center">
                                    <button 
                                        onClick={() => handleRating(selectedProduct.id, 'dislike')} 
                                        className={`w-24 h-24 rounded-full bg-white shadow-xl flex items-center justify-center text-zinc-400 hover:scale-110 active:scale-95 transition-all mb-6 group border ${userVote === 'dislike' ? 'border-zinc-800 bg-zinc-50 ring-4 ring-zinc-100' : 'border-zinc-50'}`}
                                    >
                                        <svg className={`w-10 h-10 ${userVote === 'dislike' ? 'fill-zinc-800' : 'fill-none'}`} stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <span className={`text-2xl font-black tracking-tighter ${userVote === 'dislike' ? 'text-zinc-800' : 'text-zinc-400'}`}>
                                        {ratings[selectedProduct.id]?.dislikes || 0}
                                    </span>
                                </div>
                            </div>
                        </div>
