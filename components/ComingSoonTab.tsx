...
const ComingSoonTab: React.FC<ComingSoonTabProps> = ({ items }) => {
    ...
    useEffect(() => {
        const stored = localStorage.getItem('1AIX_PUBLISHED_TKDN');
        if (stored) {
            setPublishedAiData(JSON.parse(stored));
        }
    }, []);

    // Original upcoming devices filter
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full border-collapse">
                            ...
                            <tbody className="divide-y divide-zinc-50">
                                {/* AI Published Data */}
                                {publishedAiData.length > 0 && publishedAiData.map((item, idx) => (
                                    <tr key={`ai-${idx}`} className="hover:bg-blue-50/50 transition-colors bg-blue-50/20">
                                        ...
                                        <td className="px-6 py-2 font-mono text-[9px] text-zinc-400">
                                            {item.cert_number}
                                        </td>
                                        <td className="px-6 py-2">
                                            <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 border rounded-sm shadow-sm ${item.status === 'RELEASED' ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-[#fef3c7] border-[#fde68a] text-[#d97706]'}`}>
                                                ...
                                                <span className="text-[8px] font-black uppercase tracking-wider">
                                                    {item.status === 'RELEASED' ? 'SUDAH RILIS' : 'UPCOMING'}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-2">
                                            <span className="text-[11px] font-black text-zinc-900 underline decoration-blue-500 decoration-2 underline-offset-4">{item.tkdn_score}%</span>
                                        </td>
                                        <td className="px-6 py-2 text-[10px] font-bold text-zinc-400 uppercase tracking-tight">
                                            {item.cert_date}
                                        </td>
                                    </tr>
                                ))}
                                ...
                                {upcoming.map(phone => (
                                    <tr key={phone.id} className="hover:bg-zinc-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-2 font-mono text-[9px] text-zinc-400">
                                            {phone.postel_cert || 'N/A'}
                                        </td>
                                        <td className="px-6 py-2">
                                            <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-[#fef3c7] border border-[#fde68a] text-[#d97706] rounded-sm shadow-sm group cursor-help">
                                                <svg className="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                <span className="text-[8px] font-black uppercase tracking-wider">UPCOMING</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-2">
                                            <span className="text-[11px] font-black text-zinc-900 underline decoration-blue-500 decoration-2 underline-offset-4">{phone.tkdn_score}%</span>
                                        </td>
                                        <td className="px-6 py-2 text-[10px] font-bold text-zinc-400 uppercase tracking-tight">
                                            {phone.launch_date_indo}
                                        </td>
                                    </tr>
                                ))}
                                ...
                                {publishedAiData.length === 0 && upcoming.length === 0 && (
                                    <tr>
                                        <td colSpan={5} className="px-6 py-24 text-center text-zinc-300 font-black uppercase text-xs tracking-widest italic">
                                            Belum ada data sertifikasi terbaru di monitor.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
