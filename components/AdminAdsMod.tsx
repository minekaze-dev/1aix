...
// Komponen dipindahkan ke luar agar tidak kehilangan fokus saat pengetikan (re-mount prevention)
const AdEditor = ({ 
  type, 
  config, 
  onUpdateLocal, 
  ...
  type: 'header' | 'article' | 'sidebar', 
  config: AdConfig,
  onUpdateLocal: (type: string, field: keyof AdConfig, value: string) => void,
  onSave: (type: 'header' | 'article' | 'sidebar') => void
}) => {
        ...
        <div className="space-y-6">
          <div className="space-y-4">
            ...
            <input 
              ...
              onChange={(e) => onUpdateLocal(type, 'image_url', e.target.value)}
          ...
          <div className="space-y-4">
            ...
            <input 
              ...
              onChange={(e) => onUpdateLocal(type, 'target_url', e.target.value)}
              className="w-full bg-[#f8fafc] border border-zinc-100 p-4 rounded-sm text-sm font-bold outline-none focus:border-blue-500 transition-all"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <input 
                type="text" 
                value={config.title || ''}
                onChange={(e) => onUpdateLocal(type, 'title', e.target.value)}
                className="w-full bg-[#f8fafc] border border-zinc-100 p-4 rounded-sm text-sm font-bold outline-none focus:border-blue-500 transition-all"
              />
            </div>
            <div className="space-y-4">
              ...
              <input 
                ...
                onChange={(e) => onUpdateLocal(type, 'subtitle', e.target.value)}
                className="w-full bg-[#f8fafc] border border-zinc-100 p-4 rounded-sm text-sm font-bold outline-none focus:border-blue-500 transition-all"
              />
            </div>
          </div>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto animate-in fade-in duration-500 pb-20">
      ...
      <div className="lg:hidden space-y-12 px-2">
        <AdEditor type="header" config={ads.header} onUpdateLocal={handleUpdateLocal} onSave={handleSave} />
        <AdEditor type="article" config={ads.article} onUpdateLocal={handleUpdateLocal} onSave={handleSave} />
        <AdEditor type="sidebar" config={ads.sidebar} onUpdateLocal={handleUpdateLocal} onSave={handleSave} />
      </div>
    </div>
  );
};
