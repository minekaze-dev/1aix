      ...
      <div className="bg-white border border-zinc-100 rounded-2xl shadow-sm overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-zinc-50">
            {loading ? (<tr><td colSpan={5} className="px-8 py-10 text-center animate-pulse">Memuat Artikel Database...</td></tr>) : articles.length > 0 ? articles.map(article => (
              <tr key={article.id} className="hover:bg-zinc-50 transition-colors"><td className="px-8 py-5"><div className="flex items-center gap-4"><div className="w-12 h-8 bg-zinc-100 rounded overflow-hidden shadow-sm"><img src={article.cover_image_url} alt="" className="w-full h-full object-cover" /></div><div className="text-[11px] font-black text-zinc-900 uppercase truncate max-w-xs">{article.title}</div></div></td><td className="px-8 py-5"><div className="flex flex-wrap gap-1">{(article.categories || []).map(cat => (<span key={cat} className="text-[8px] font-black text-blue-600 uppercase bg-blue-50 px-1.5 py-0.5 rounded-sm border border-blue-100">{cat}</span>))}</div></td><td className="px-8 py-5"><span className={`text-[8px] font-black px-2 py-1 rounded-sm uppercase ${article.status === 'PUBLISHED' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>{article.status}</span></td><td className="px-8 py-5 text-[10px] font-black text-zinc-400">{article.publish_date}</td><td className="px-8 py-5 text-right"><div className="flex justify-end gap-2"><button onClick={() => onEditArticle(article)} className="p-2 text-zinc-400 hover:text-blue-600 transition-colors"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L15.232 5.232z"></path></svg></button><button onClick={() => handleDelete(article.id)} className="p-2 text-zinc-400 hover:text-red-600 transition-colors"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></div></td></tr>
            )) : (<tr><td colSpan={5} className="px-8 py-20 text-center text-zinc-300 font-black uppercase text-[10px] tracking-widest">Belum ada artikel di database.</td></tr>)}
          </tbody>
        </table>
      </div>
      {showCategoryModal && (
