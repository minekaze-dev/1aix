      ...
      <header className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
            <button 
                ...
                className="flex items-center gap-3 px-6 py-3 bg-zinc-900 text-white text-[10px] font-black uppercase tracking-widest rounded-sm shadow-xl hover:bg-zinc-800 transition-all transform active:scale-95 border border-zinc-700"
            >
                <svg className="w-4 h-4 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M7 7h.01M7 3h5a2 2 0 012 2v5a2 2 0 01-2 2H7a2 2 0 01-2-2V5a2 2 0 012-2z"></path></svg>
                KATEGORI
            </button>
            <button 
                onClick={onCreateArticle}
                className="flex items-center gap-3 px-6 py-3 bg-[#ef4444] text-white text-[10px] font-black uppercase tracking-widest rounded-sm shadow-xl hover:bg-red-600 transition-all transform active:scale-95"
            >
      ...
      {showCategoryModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-6">
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="bg-zinc-900 px-8 py-6 flex justify-between items-center">
                    <h3 className="text-white text-[12px] font-black uppercase tracking-widest">KATEGORI ARTIKEL</h3>
                    <button onClick={() => setShowCategoryModal(false)} className="text-zinc-400 hover:text-white transition-colors">
                ...
                <div className="p-8">
                    <div className="flex gap-2 mb-8">
                        <input 
                            ...
                            className="flex-1 px-4 py-3 bg-zinc-50 border border-zinc-100 rounded text-[11px] font-black uppercase outline-none focus:border-red-500 transition-colors"
                        />
                        <button 
                            onClick={handleAddCategory}
                            className="px-6 py-3 bg-blue-600 text-white text-[10px] font-black uppercase rounded shadow-lg shadow-blue-500/20 active:scale-95"
                        >
                            TAMBAH
                        </button>
                    </div>
                    ...
                    <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 scrollbar-hide">
                        {categories.map((cat, idx) => (
                            <div key={idx} className="flex items-center justify-between p-3 bg-zinc-50 border border-zinc-100 rounded group hover:border-zinc-300 transition-colors">
                                {editingCat?.index === idx ? (
                                    <div className="flex-1 flex gap-2">
                                        <input 
                                            ...
                                            className="flex-1 px-2 py-1 bg-white border border-blue-500 rounded text-[11px] font-black uppercase outline-none"
                                            autoFocus
                                        />
                                        <button onClick={handleUpdateCategory} className="text-blue-600 font-black text-[10px] uppercase">SAVE</button>
                                        <button onClick={() => setEditingCat(null)} className="text-zinc-400 font-black text-[10px] uppercase">ESC</button>
                                    </div>
                                ) : (
                                    <>
                                        <span className="text-[11px] font-black uppercase text-zinc-700">{cat}</span>
                                        <div className="flex gap-4">
                                            <button onClick={() => setEditingCat({index: idx, name: cat})} className="text-blue-500 hover:text-blue-700 transition-colors">
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L15.232 5.232z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path></svg>
                                            </button>
                                            <button onClick={() => handleDeleteCategory(idx)} className="text-zinc-300 hover:text-red-500 transition-colors">
