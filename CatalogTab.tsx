...
const CatalogTab: React.FC<CatalogTabProps> = ({ 
    ...
    // LOGIKA DETEKSI SLUG URL - MEMASTIKAN DETAIL TERBUKA SAAT DIKLIK ATAU VIA URL
    useEffect(() => {
        const handleSlugDetection = () => {
            const hash = window.location.hash;
            if (hash.startsWith('#/katalog/') && hash.length > 10) {
                const slug = hash.replace('#/katalog/', '');
        ...
        handleSlugDetection();
        window.addEventListener('hashchange', handleSlugDetection);
        return () => window.removeEventListener('hashchange', handleSlugDetection);
    }, [items]);
            ...
            <div className="flex-grow">
                {selectedProduct ? (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="flex flex-col md:flex-row gap-8 items-start">
                            <div className="md:w-[240px] flex-shrink-0">
                                ...
                                <div className="mt-2 text-center"><span className="text-[7px] font-bold text-zinc-300 uppercase tracking-widest leading-none">Image: GSMArena</span></div>
                            </div>
                            <div className="flex-1 flex flex-col w-full min-w-0">
                                <div className="mb-6">
                                    ...
                                    <div className="py-6 mb-6 border-y border-zinc-100 flex flex-col items-center justify-center bg-zinc-50/50 rounded-sm">
                                        ...
                                        <div className="text-3xl font-black text-blue-600 tracking-tighter italic">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(selectedProduct.price_srp).replace('Rp', 'Rp ')}</div>
                                        <div className="mt-3 text-[9px] font-bold text-zinc-400 uppercase tracking-widest text-center px-6 leading-relaxed max-w-[500px]">* HARGA PASAR DAPAT BERUBAH SEWAKTU-WAKTU TERGANTUNG WILAYAH DAN KEBIJAKAN TOKO RETAIL. DATA SRP RESMI SAAT PELUNCURAN.</div>
                                    </div>
                                    <div className="flex flex-col gap-3">
                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="flex items-center justify-center py-3 bg-zinc-900 text-white font-black uppercase text-[9px] tracking-widest rounded-sm shadow-md cursor-default border border-zinc-800">SPESIFIKASI</div>
                                            <button onClick={() => onCompareProduct ? onCompareProduct(selectedProduct.id) : (window.location.hash = '#/bandingkan')} className="flex items-center justify-center py-3 bg-blue-600 text-white font-black uppercase text-[9px] tracking-widest hover:bg-blue-700 transition-all rounded-sm shadow-md active:scale-[0.98]">COMPARE</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="space-y-8 animate-in fade-in duration-500">
                        ...
                        {filtered.length === 0 && <div className="py-20 text-center text-zinc-300 font-black uppercase tracking-widest italic">Tidak ada gadget yang sesuai filter pencarian.</div>}
                    </div>
                )}
            </div>
        </div>
